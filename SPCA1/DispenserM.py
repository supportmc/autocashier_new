...
def SendDispenser(transaction):
    ...
    serie = serial.Serial(puerto, 38400,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS, timeout=1 )    
...
def sacarTarjeta(tipo):
    ...
    while tarjeta=='':
        ...
        sleep(0.1)
        if tipo=='M':
            tarjeta=str(SendDispenser('6'))
        else:
            tarjeta=str(SendDispenser('16'))
        if tarjeta !='':
