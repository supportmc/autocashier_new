    ...
    try:
        ...
        if conn:
            try:
                c=conn.cursor()
                v=event
                sql=("insert into TBL_Close_Transaction (ID_History_Event,Reader,MPL,MPL_Number,System,Total_amount_local_currency,open_cash,charge_time) values('%s','%s','%s','%s','%s','%s','%s','%s')")%(v['Id_History_Event'],v['Reader'],v['MPL'],v['MPL_Number'],v['System'],v['Total_amount_local_currency'],v['open_cash'],v['charge_time'])
                c.execute(sql)
                conn.commit()
                c.close()
                conn.close()
